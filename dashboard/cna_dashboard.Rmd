...
# reading data
dat_seg <- readr::read_tsv(fs::dir_ls(path = here(), recursive = T, glob= "*uber*seg.txt")) %>% janitor::clean_names()
dat_seg_cp <- dat_seg %>% dplyr::select(-chrom, -chrompos, -abspos)

dat_bin <- readr::read_tsv(fs::dir_ls(path = here(), recursive = T, glob= "*uber*bin.txt")) %>% janitor::clean_names()
...
# hackish way to get sample names match with clean_names result. In the future it can be substitute by make_clean_names when janitor function goes to CRAN
cleared_names_stats <- setNames(1:length(dat_stats$sample_name), dat_stats$sample_name) %>% janitor::clean_names()

dat_stats <- dat_stats %>% 
  mutate(sample_name = names(cleared_names_stats)) %>% 
...
### PCR Duplicates (Median)

```{r calculating_pcr_duplicates}
dups <- round(dat_stats_metrics$duplicates*100,2)
gauge(dups, min = 0, max = 100, symbol = '%', gaugeSectors(
  success = c(0, 29), danger = c(30, 100)
))
```
...
# hackish way to get sample names match with clean_names result. In the future it can be substitute by make_clean_names when janitor function goes to CRAN
cleared_names <- setNames(1:length(dat_ploidy$sample_name), dat_ploidy$sample_name) %>% janitor::clean_names()

dat_ploidy <- dat_ploidy %>% 
  dplyr::mutate(sample_name = names(cleared_names))
  

# obtaining SNN clusters labels to use as colors, matching with cell name and plotting
...
ratios_data_l_gat <- tidyr::gather(data = ratios_data %>% dplyr::select(1:30, abspos), key = "cell", value = "ratios_rat", -abspos)
long_seg_t_gat <- tidyr::gather(data = dat_seg_cp %>% dplyr::select(1:30, abspos), key = "cell", value = "seg_mean", -abspos)
