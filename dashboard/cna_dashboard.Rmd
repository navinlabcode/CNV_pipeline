...
# reading data
dat_seg <- read.table(fs::dir_ls(path = paste0(here(),"/",params$outdir), recurse = T, glob="*uber*seg.txt"),sep="\t",header=T) %>% janitor::clean_names()
dat_seg_cp <- dat_seg %>% dplyr::select(-chrom, -chrompos, -abspos)

dat_bin <- read.table(fs::dir_ls(path = paste0(here(),"/",params$outdir), recurse = T, glob="*uber*bin.txt"),sep="\t",header=T) %>% janitor::clean_names()
dat_stats <- read.table(fs::dir_ls(path = paste0(here(),"/",params$outdir), recurse = T, glob="*stat*metrics.txt"),sep="\t",header=T) %>%
  janitor::clean_names() %>%
...
dat_rat <- read.table(fs::dir_ls(path = paste0(here(),"/",params$outdir), recurse = T, glob= "*uber*ratio.txt"),sep="\t",header=T) %>%janitor::clean_names()
