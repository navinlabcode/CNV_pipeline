...
dat_stats <- read.table(list.files(path = paste0(here(),"/",params$outdir), pattern="*stat\\_metrics.txt",recursive = T,full.names=T),sep="\t",header=T) %>%
  janitor::clean_names() %>%
...
dat_rat <- read.table(list.files(path = paste0(here(),"/",params$outdir), pattern="*\\.ratio.txt",recursive = T,full.names=T),sep="\t",header=T) %>%janitor::clean_names()
...
Sample_well=rep("PeakA",nrow(dat_seg_order))
Sample_well[grep(rownames(dat_seg_order),pattern = "Ctrl",ignore.case = T)]="Ctrl"
Sample_well[grep(rownames(dat_seg_order),pattern = "_D_",ignore.case = T)]="PeakD"

Cells=rep("ctrl",nrow(dat_seg_order))
Cells[grep(rownames(dat_seg_order),pattern = "_BL_",ignore.case = T)]="PR421_BL"
Cells[grep(rownames(dat_seg_order),pattern = "_RP_",ignore.case = T)]="PR421_RP"
#Cells[grep(rownames(dat_seg_order),pattern = "SC89_NKI_9P",ignore.case = T)]="NKI9P_PM2179"
#Cells[grep(rownames(dat_seg_order),pattern = "post",ignore.case = T)]="KI01_post"
col<-c("blue","orange","grey")
names(col)<-c("PR421_BL","PR421_RP","ctrl")
pcol<-c("yellow","green","red")
names(pcol)<-c("Ctrl","PeakD","PeakA")
heat_row_col <- rowAnnotation(
	     Cells = Cells,
             Sample_well=Sample_well,
	     col = list(Cells = col,Sample_well=pcol))
#	     col = list(Cells = col))
