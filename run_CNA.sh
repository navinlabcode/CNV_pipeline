...
run_bowtie_varbin(){
	...
	then
		...
		echo "perl $bin/run_bowtie2_varbin.pl -fq1 $s1_input -fq2 $s2_input -samdir $sam_folder -bamdir $bam_folder -sortdir $sort_folder -vb_dir $vbdir_folder -stat_dir $stat_folder -res $res" >> $s1_tmp
	else
	s1_dir=`dirname $s1_input`
