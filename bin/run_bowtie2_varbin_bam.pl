...
die `pod2text $0` if ($Help || !$bam_dir);
...
#	$fname =~ /^(.*?)\.(.*?)\.bam/;
	$pre=$1;
	$cmd="$samtools view $file -@ 6 >$sortdir/$pre.sam";
	print "$cmd\;";
#	system("$cmd");
#Creates varbins files
	$cmd="$varbin_python $sortdir/$pre.sam $vb_dir\/$pre\.vb $stat_dir\/$pre\.stat\.txt $chrominfo $bins";
	print "$cmd\n";
#	system("$cmd");
}
